var Mine = function  (ele,faceele,panewidth,paneheight,minenum,tagele,timeele) {
 ...
 Mine.prototype = {
    ...
    this.faceele.src = "http://www.saolei.org/res/face_normal.bmp";
	...
	triggerClick:function(pos,e){
		...
		if(theCell.isMine == true){// 如果点到的这个是雷，就判输。显示所有雷
			this.faceele.src = "http://www.saolei.org/res/face_fail.bmp";
			this.showMine();
		...
		if(openNum == okNum){
			this.faceele.src = "http://www.saolei.org/res/face_success.bmp";
			alert("you win!");
			clearInterval(this.timer);
			this.ele.onmouseup = '';
			this.ele.onmousedown = '';
	...
	drawCell:function(pos,type){//绘制不同种类的格子。
		...
		var image = new Image();
		var src;
		var srcArr = ["http://www.saolei.org/res/blank.bmp","http://www.saolei.org/res/0.bmp","http://www.saolei.org/res/flag.bmp","http://www.saolei.org/res/ask.bmp","http://www.saolei.org/res/mine.bmp","http://www.saolei.org/res/blood.bmp","http://www.saolei.org/res/error.bmp"];
	...
	drawNum:function(pos,num){//绘制数字
		...
		var cxt = this.cxt;
		var image = new Image();
		image.src = "http://www.saolei.org/res/"+num+".bmp";
		image.onload = function(){
			cxt.drawImage(image,area[0],area[1],16,16);
		}
	},
	...
	numToImage:function (num,ele){
		...
		for (var i = 0,eLen=ele.length; i < eLen; i++) {
			ele[i].src="http://www.saolei.org/res/d"+num.toString().charAt(i)+".bmp";
		};
